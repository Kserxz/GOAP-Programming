...
public class GoapPlanner : IGoapPlanner
{
    ...
    {
        // Сортируем цели по убыванию приоритета
        List<AgentGoal> orderedGoals = goals
            .Where(g => g.DesiredEffects.Any(b => !b.Evaluate()))
            .OrderByDescending(g => g == mostRecentGoal ? g.Priority - 0.01 : g.Priority)
            .ToList();

        // Пробуем решить цели по очереди
        foreach (var goal in orderedGoals)
        {
            Node goalNode = new Node(null, null, goal.DesiredEffects, 0);

            // Если можем найти путь до цели, возвращаем его
            if (FindPath(goalNode, agent.actions))
            {
                // Если goalNode не может найти путь до цели, ищем путь для другой цели
    ...
    bool FindPath(Node parent, HashSet<AgentAction> actions)
    {
        foreach (var action in actions)
        {
            ...
            // Убрать все эффекты, считающиеся правдой, т.к. нет действий, они уже выполнены
            requiredEffects.RemoveWhere(b => b.Evaluate());

            // Если не осталось эффектов для выполнения, мы имеем план
            ...
            if (action.Effects.Any(requiredEffects.Contains))
            {
                ...
                var newAvaliableActions = new HashSet<AgentAction>(actions);
                newAvaliableActions.Remove(action);

                var newNode = new Node(parent, action, newRequiredEffects, parent.Cost + action.Cost);

                // Изучим новую ноду рекурсией
                if (FindPath(newNode, newAvaliableActions))
                {
                    parent.Leaves.Add(newNode);
                    newRequiredEffects.ExceptWith(newNode.Action.Preconditions);
                }

                // Если все эффекты на этой глубине выполнимы
